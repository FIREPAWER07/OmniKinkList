var log = (val, base) => Math.log(val) / Math.log(base)
var strToClass = (str) => {
  var className = ""
  str = str.toLowerCase()
...
$(() => {
  ...
  inputKinks = {
    ...
    createKink: (fields, kink) => {
      ...
      if (kink.kinkDesc) {
        showDescriptionButton(kink.kinkDesc, kinkLabel)
      }
  ...
  ;(() => {
    ...
    inputKinks.inputPopup = {
      ...
      showIndex: (index) => {
        ...
        if (currentKink.kink.desc) {
          showDescriptionButton(currentKink.kink.desc, label)
        }
        $options.append($currentKink)
    ...
    $("#InputCurrent .closePopup, #InputOverlay").on("click", () => {
      $popup.fadeOut()
    })
  })()
})
