...
export type PreferenceLevel = "favorite" | "like" | "indifferent" | "maybe" | "limit" | "not-entered"

export interface UserPreference {
  itemName: string
  level: PreferenceLevel
  notes?: string
  dualLevel?: PreferenceLevel
}

export const preferenceColors: Record<PreferenceLevel, string> = {
  favorite: "bg-[var(--color-favorite)]",
  like: "bg-[var(--color-like)]",
  indifferent: "bg-[var(--color-indifferent)]",
  maybe: "bg-[var(--color-maybe)]",
  limit: "bg-[var(--color-limit)]",
  "not-entered": "bg-[var(--color-not-entered)]",
}

export const preferenceLabels: Record<PreferenceLevel, string> = {
  favorite: "Favorite",
  like: "Like",
...
// Parse kink list data from text format
export function parseKinkListData(data: string): KinkCategory[] {
  const lines = data.split("\n").filter((line) => line.trim())
  const categories: KinkCategory[] = []
  let currentCategory: KinkCategory | null = null
  ...
  for (const line of lines) {
    if (line.startsWith("#")) {
      ...
      // Subcategory - check for dual preference indicators
      ...
      if (currentCategory) {
        ...
        let dualLabels: { first: string; second: string } | undefined
...
// Extended kink list data combining all variants
export const extendedKinkListData = `
#Bodies
(General)
* Skinny
...
export const classicKinkListData = `
#Bodies
(General)
* Skinny
* Chubby
...
export const detailedKinkListData = `
#Bodies
(General)
* Skinny
? Expresses an interest in characters that are waifish, thin, or slender.
...
export function getKinkListData(listType: string): string {
  switch (listType) {
    case "classic":
      return classicKinkListData
    case "detailed":
      return detailedKinkListData
    case "extended":
    default:
      return extendedKinkListData
  }
}
...
export const categoryExplanations: Record<string, string> = {
  ...
  "Roles & Dynamics": "Character roles, gender expressions, and power dynamics you enjoy playing or experiencing.",
  "Group Scenarios": "Partner configurations and group size preferences for sexual encounters.",
  General: "Basic sexual activities, romantic interactions, and common intimate acts.",
  ...
  Toys: "Sex toys, devices, and objects used for sexual stimulation and enhancement.",
  Domination: "Power exchange dynamics, control scenarios, and dominant/submissive relationships.",
  "No consent": "Fantasy scenarios involving non-consensual elements (all fantasy roleplay only).",
  ...
  "Touch & Stimulation": "Sensory experiences, worship activities, and various forms of physical stimulation.",
  "Misc. Fetish": "Various specialized fetishes and kinks that don't fit other categories.",
  ...
  Surrealism:
    "Fantasy elements, non-human characters, supernatural scenarios, and otherworldly beings including shapeshifters, mythical creatures, and elemental entities.",
  Medium:
    "Communication platforms and methods for roleplay interactions, including modern messaging apps, social media, and specialized platforms.",
  "Time-Scale":
    "Timing preferences for scenes and response expectations, from real-time sessions to long-term storytelling with flexible pacing options.",
}
